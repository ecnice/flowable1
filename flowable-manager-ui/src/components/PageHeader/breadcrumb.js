...
export default class BreadcrumbView extends PureComponent {
  state = {
    breadcrumb: null,
    hasInitBreadcrumb: false, // 是否初始化过
  };
  ...
  componentDidUpdate(preProps) {
    ...
    const breadcrumbLength = Object.keys(breadcrumbNameMap).length;

    if (!breadcrumbLength || !location || !preProps.location) {
      return;
    }
    ...
    // 因为菜单数据是异步获取的，不能保证是初始化就有了的。
    // 如果还没有初始化过面包屑，而且面包屑数量>0，就去更新面包屑，同时更新state.hasInitBreadcrumb
    if (!this.state.hasInitBreadcrumb) {
      this.setState({
        hasInitBreadcrumb: true,
      });
      this.getBreadcrumbDom();
    }
  }
